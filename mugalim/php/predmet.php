...
<link rel="icon" href="../img/favicon.ico" type="image/x-icon">
  ...
  <body>
   ...
   <div class="container-lg">
      ...
      <h4>Предметтер:</h4>
    <?
      if ($_SESSION["status"] == "mug-di" || $_SESSION["status"] == "mug-za") echo '
      <form method="post">
                <div class="form-group">
                  <input type="text" class="form-control" name="aty" placeholder="Предметтин аталышы" value="" required>
                </div>
                <br>
                <div class="row">
    ...
    <a href="../" class="btn btn-danger"> Артка </a>
      ...
      if (isset($_POST["submit_news"])) {
        $a = $_POST["aty"];
        $conn->query("INSERT INTO predmet (aty) VALUES('$a')");
        reload();
      }
      function reload() {
        echo'
        <script>
            function Load() {
                window.location.replace("predmet.php");
            }
            setTimeout(Load,1);
        </script>
        ';
